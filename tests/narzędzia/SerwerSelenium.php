...
class SerwerSelenium {
    ...
    public function zakończDziałanieSerweraSelenium() {
        ...
        print_r(PHP_EOL . 'Zakończono działanie serwera Selenium pod adresem: ' . 'http://' . SELENIUM_SERWER_HOST . ':' . SELENIUM_SERWER_PORT);
    }
    ...
    private function czekajNaUruchomienieSerweraSelenium() {
        for ($i = 0; $i < 30; $i++) {
            $kod_odpowiedzi = @get_headers('http://' . SELENIUM_SERWER_HOST . ':' . SELENIUM_SERWER_PORT)[0];
            if (strpos($kod_odpowiedzi, '200') !== false) {
                print_r(PHP_EOL . 'Uruchomiono serwer Selenium pod adresem: ' . 'http://' . SELENIUM_SERWER_HOST . ':' . SELENIUM_SERWER_PORT);
                return;
            }
            sleep(1);
        }
